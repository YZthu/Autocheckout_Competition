...
class WeightSensor:
    ...
    def weight_change_detection(self, total_detected_queue, detected_weight_event_queue):
        ...
        for kk in np.arange(1, len(self.value)):
            ...
            if(continue_count > CONTINUE_TH) & (state==0):
                pre_weight = np.mean(self.value[(kk-4): (kk)])
                state =1
            ...
            if (continue_count > CONTINUE_TH) & (state ==2):
                ...
                #print(self.timestamp-1580250250)
    ...
    def value_update(self, total_detected_queue, detected_weight_event_queue, val, timestamp):
        ...
        for kk in np.arange(1, len(val)-10, 10):
            tmp_val = val[kk:kk+10]
            tmp_ts = timestamp[kk:kk+10]
            mean_ts = np.mean(tmp_ts)
            mean_wv = np.mean(tmp_val)
            new_ts.append( mean_ts)
            new_val.append( mean_wv)
        ...
        if self.number == 49:
            ...
            print(new_val)
"""
        self.value = np.concatenate((self.value, np.array(new_val)),axis = 0)
        self.timestamp = np.concatenate((self.timestamp, np.array(new_ts)), axis=0)
        if len(self.value) >8*10: #store 20 seconds data
            del_num = len(self.value) - 8*10
    ...
    if changed_weight > 5:
        ...
        if len(current_shopping_list) < 1:
            ...
            for tmp_kk in range(len(current_shopping_list)):
                ...
                item_condidate.append(tmp_item_info)
            print('item_condidate')
            print(item_condidate)
        ...
        if changed_weight < -5:
            ...
            if sensor_total_number == 1:
                ...
                tmp_item_list = weight_sensor_item_info_queue[sensor_number[0]-1]
                for kk in range(len(tmp_item_list)):
                    item_condidate.append(tmp_item_list[kk])
            else:
                for tmp_s_num in range(len(sensor_number)):
                    tmp_sensor_num = sensor_number[tmp_s_num-1]
                    tmp_item_list = weight_sensor_item_info_queue[tmp_sensor_num-1]
                    for kk in range(len(tmp_item_list)):
                        item_condidate.append(tmp_item_list[kk])
                    
        if len(item_condidate) > 0:  #the weight sensor contian useful info
            item_fin_name, item_fin_number, item_fin_price,  item_per_weight = most_similar_item_estimation(item_condidate, np.abs(changed_weight), 5)
        else:
            item_condidate = out_sensor_item_info
            item_fin_name, item_fin_number, item_fin_price,  item_per_weight = most_similar_item_estimation(item_condidate, np.abs(changed_weight), 2)

...
class Vision_module:
    def __init__(self, camera_id, image_folder, shelf_bbox_list, init_timestamp, fps):
    ...
    def update_frame(self, pose_estimator, signal):
        ...
        if signal > 0:
            ...
            for det_id in range(num_dets):
                ...
                print('Person bbox:', bbox_x1y1x2y2)
    ...
    def add_person_feature(self, pose_estimator, timestamp):
        ...
        curr_img_id = get_img_id_from_timestamp(timestamp, self.init_timestamp, self.fps)
        print('frame:', curr_img_id)
    ...
    def get_visual_info(self, pose_estimator, timestamp, shelf_id):
        ...
        for bbox_det_dict in self.bbox_dets_list:
            ...
            if isPoiWithinPoly([x3_r,y3_r], [shelf_bbox]):
                ...
                continue # if right hand picks, pass left hand
            ...
            if isPoiWithinPoly([x3_l,y3_l], [shelf_bbox]):
                ...
                print("ID:"+str(person_id)+" picks on shelf "+ str(gondola_num)+ "-" + str(shelf_num) +" at frame "+str(self.img_id))

        return pick_result

def get_shelf_position(shelf_id):
    gondola_id = shelf_id//(15*7)
    shelf_id = (shelf_id%(15*7))//15
    return gondola_id, shelf_id

def get_img_id_from_timestamp(timestamp, init_timestamp, fps):
    return max(0,int((timestamp - init_timestamp)*fps)) #or it may not align well...
...
def get_track_id_person_reID(img_path, bbox_det, img_id, camera_id):
    ...
    print('Person ID and similarity:', np.argmax(sim_all[0])+1, max(sim_all))
...
def person_reID_add(img_path, bbox_det, img_id, camera_id):
    ...
    person_reID_feature = extract_person_feature(cropImg_Tensor)
    person_reID_list = np.append(person_reID_list,person_reID_feature, axis=0)
    print('Person codebook len:', len(person_reID_list))
    return 

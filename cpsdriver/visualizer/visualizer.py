...
def show_all_from_standard_json(json_file_path, classes, joint_pairs, joint_names, img_folder_path = None, output_folder_path = None, flag_track= False):
    ...
    for det in dets:
        ...
        for candidate in candidates:
            ...
            # optional 2: show shelf bbox
            img_id = int(python_data["image"]["name"][5:-4])
            trigger_number = [190, 315]
            trigger = True if img_id in trigger_number else False 
            img, pick_results = draw_shelf_bbox(img, joints, joint_pairs, joint_names, track_id, trigger, img_id, img_path, pick_results)
    ...
    for i,result in enumerate(pick_results):
        result_output_path = os.path.join(output_folder_path, 'result_ID{}_img{}.jpg'.format(result['track_id'],result['img_id']))
        cv2.imwrite(result_output_path, result['memory_item_img'])
    return
